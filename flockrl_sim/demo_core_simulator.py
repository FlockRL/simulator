        ...
        if info["done"]:
            ...
            if info.get("collisions"):
                for col in info["collisions"]:
                    print(f"    - Collision with {col.collision_type} (drone {col.drone_id})")
            break
...
def demo_perception_system():
    ...
    for i, obs in enumerate(observations):
        ...
        print(f"  - Ray-cast ranges: {obs.ranges.shape} (min={obs.ranges.min():.2f}m, max={obs.ranges.max():.2f}m)")
    ...
    for ep in range(3):
        ...
        for step in range(200):
            ...
            if info["done"]:
                ...
                print(f"  Final goal distance: {info['episode_stats']['final_goal_distance']:.2f}m")
                break
...
def demo_save_and_record():
    ...
    state = simulator.start_run(initial_state, {
        "demo": "recording",
        "description": "Test simulation recording"
    })
