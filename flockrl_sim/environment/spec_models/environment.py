...
class EnvironmentSpec(BaseModel):
    ...
    @classmethod
    def validate_bounds(cls, bounds: Tuple[float, float, float, float, float, float]) -> Tuple[float, float, float, float, float, float]:
        x_min, x_max, y_min, y_max, z_min, z_max = bounds
        if not (x_min < x_max and y_min < y_max and z_min < z_max):
            raise ValueError(f"Invalid bounds: ({x_min}, {x_max}, {y_min}, {y_max}, {z_min}, {z_max}) — must have min < max in each dimension")
        return bounds

    @field_validator("obstacles")
    @classmethod
    def validate_unique_ids(cls, obstacles: List[Union[WallSpec, ClutterSpec]]) -> List[Union[WallSpec, ClutterSpec]]:
