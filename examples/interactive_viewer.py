...
VIEWER_CONTROLS = {
    ...
    "unified": [
        ...
        "  ← → : Cycle through environments",
        "  ↑ ↓ : Move 2D slice up/down (z-axis)",
        "  Mouse (3D panel): Left-drag=rotate, Right-drag=zoom",
        "  Q   : Quit",
    ],
}
...
class ViewerUnified(BaseViewer):
    ...
    def render(self):
        elev, azim = (
            (self.ax_3d.elev, self.ax_3d.azim)
            if hasattr(self.ax_3d, "elev")
            else (20, -60)
        )
        self.render_2d()
        self.render_3d()
